...
const page: FC<PageProps> = async ({ params }) => {
    ...
    return <div className="container mt-10 pb-32">
        <div className='flex mb-10'>
            <h3 className='px-5 border-r border-muted-foreground font-monument tracking-wider text-transparent bg-clip-text bg-gradient-to-b from-[#2BADFD] to-[#1570EF]'>SUNBELT</h3>
            <h3 className='px-5 border-l border-r border-muted-foreground'>Blogs and News</h3>
            <h3 className='px-5 border-l border-muted-foreground'>Article</h3>
        </div>

        <h1 className="font-monument text-2xl tracking-wider mb-10">{doc.title}</h1>

        <div className="flex flex-col lg:flex-row-reverse w-full gap-5">
            <div className="flex flex-col w-80 gap-2">
                <h1 className="font-monument text-primary mb-3 border-t-2 pt-4">Table of content</h1>
                {doc.headings.map((heading: any, index: number) => (
                    <Link href={`#${heading.slug}`} className="p-2 hover:bg-accent">{heading.text}</Link>
                ))}
            </div>
